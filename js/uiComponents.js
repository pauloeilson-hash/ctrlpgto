...
constructor() {
    ...
    this.domCache = {};
    this.backupControlsSetup = false; // ← NOVA FLAG
 ...
 initializeComponents() {
    ...
    // CORREÇÃO: Mudar para setupBackupControlsOnce
    this.setupBackupControls(); // ← ALTERADO (era setupBackupControls)
...
/**
 * Configura listeners de eventos - VERSÃO CORRIGIDA
 */
setupEventListeners() {
    ...
    // CORREÇÃO: Remover a chamada duplicada do setupBackupControls
    // this.setupBackupControls(); // ← REMOVER ESTA LINHA

    // Configura apenas os controles de backup uma vez
...
/**
 * Backup local dos dados - VERSÃO CORRIGIDA
 */
handleLocalBackup() {
    try {
        const data = dataManager.loadData();
        ...
        const filename = `backup_pagamentos_${timestamp}.json`;

        // Estrutura do backup - mesma que a restauração espera
        const backupData = {
            metadata: {
            ...
            data: data // ← ESTE É O CAMPO IMPORTANTE
        };
        ...
        this.showToast(`✅ Backup local gerado: ${filename}`, 'success');

    } catch (error) {
        console.error('❌ Erro no backup local:', error);
        this.showToast('Erro ao gerar backup local', 'error');
    }
}
...
/**
 * Configura controles de backup - VERSÃO SIMPLES
 */
setupBackupControls() {
    console.log('🔧 Configurando controles de backup...');
    
    // Remove listeners existentes dos botões de backup
    const backupButtons = [
        'btn-backup-local', 'btn-restore-local',
        'btn-google-login', 'btn-backup-drive', 'btn-restore-drive', 'btn-logout-drive'
    ];
    
    backupButtons.forEach(id => {
        const element = this.getElement(id);
        if (element) {
            const newElement = element.cloneNode(true);
            element.parentNode.replaceChild(newElement, element);
    ...
    this.getElement('btn-backup-local').addEventListener('click', () => {
        console.log('💾 Backup local clicado (único)');
        this.handleLocalBackup();
    });
    
    this.getElement('btn-restore-local').addEventListener('click', () => {
        console.log('📥 Restaurar local clicado (único)');
...
/**
 * Processa restauração de arquivo local - VERSÃO CORRIGIDA
 */
        ...
        reader.onload = (e) => {
            try {
                ...
                if (file.name.endsWith('.json')) {
                    // Backup JSON - formato correto
                    const backupData = JSON.parse(fileContent);
                    
                    // Verifica se é o formato do backup atual
                    if (backupData.data && Array.isArray(backupData.data)) {
                        data = backupData.data;
                    } 
                    // Verifica se é formato antigo (array direto)
                ...
                if (!Array.isArray(data)) {
                    throw new Error('Arquivo de backup não contém dados válidos');
                }
                
                // Valida os dados
                const validatedData = this.validateBackupData(data);
                
                if (!confirm(`Deseja restaurar ${validatedData.length} registros do backup?\n\n⚠️ ATENÇÃO: Os dados atuais serão substituídos.`)) {
                    return;
                }
                
                dataManager.saveData(validatedData);
                this.showToast(`✅ Backup restaurado! ${validatedData.length} registros carregados.`, 'success');
                this.handleDataChange();
...
validateBackupData(data) {
    if (!Array.isArray(data)) {
        throw new Error('Dados do backup devem ser um array');
    }
    
    return data.map((item, index) => {
        // Verifica campos obrigatórios
        if (!item.nome || typeof item.nome !== 'string') {
            throw new Error(`Registro ${index + 1}: Campo 'nome' inválido ou faltando`);
        }
        
        if (!item.data || !Utils.isValidDate(item.data)) {
            throw new Error(`Registro ${index + 1}: Campo 'data' inválido ou faltando`);
        }
        
        if (!item.valor || isNaN(item.valor) || item.valor <= 0) {
            throw new Error(`Registro ${index + 1}: Campo 'valor' inválido`);
        }
        
        // Garante que tem ID (gera novo se necessário)
        const validatedItem = {
            id: item.id && !isNaN(item.id) ? parseInt(item.id) : Date.now() + index,
            nome: item.nome.trim(),
            data: item.data,
...
// REMOVER ou COMENTAR esta linha final:
// window.uiComponents = new UIComponents(); // ← REMOVER ESTA LINHA